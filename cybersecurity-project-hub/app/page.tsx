...
const navItems = [
  ...
  { label: 'Write-ups', icon: FileText, count: '24' },
  { label: 'Projects', icon: FolderKanban, count: '06' },
  { label: 'Playbooks', icon: Terminal },
]
    ...
    <main className="app-shell">
      <aside className={`sidebar ${mobileNav ? 'sidebar-open' : ''}`}>
        ...
        <nav className="primary-nav" aria-label="Main navigation">
          ...
          {navItems.map(({ label, icon: Icon, count }) => <button key={label} onClick={() => { setActiveNav(label); setMobileNav(false) }} className={`nav-item ${activeNav === label ? 'active' : ''}`}><Icon size={17} /><span>{label}</span>{count && <em>{count}</em>}</button>)}
      ...
      <section className="content-area">
        ...
        <div className="page-wrap">
          <div className="page-heading"><div><p className="kicker"><span className="pulse" /> SYSTEM ONLINE / US-EAST-1</p><h1>Good morning, Jordan.</h1><p className="lede">Your security knowledge base is up to date. Here&apos;s what&apos;s happening across your workspace.</p></div><div className="heading-date"><span>MONDAY</span><strong>01 <small>SEP</small></strong><span>2026</span></div></div>
