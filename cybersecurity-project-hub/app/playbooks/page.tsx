...
export default function Playbooks() {
    ...
    return <WorkspaceShell title="Playbooks">
        ...
        <div className="playbook-grid">{playbooks.map(p => <DataCard key={p[0]} className="playbook-card">
            <div className="playbook-card-top"><div className="terminal-mark">&gt;_</div>
                <Tag tone={p[1] === 'SECURITY' ? 'orange' : 'cyan'}>{p[1]}</Tag></div>
            <h3>{p[0]}</h3>
            <div className="playbook-meta"><span><Terminal size={13} />{p[2]}</span>
                <span><Clock3 size={13} />{p[3]}</span>
            </div><div className="playbook-footer"><span>{ran === p[0] ? <><CheckCircle2 size={14} /> Started</> : p[4]}</span>
